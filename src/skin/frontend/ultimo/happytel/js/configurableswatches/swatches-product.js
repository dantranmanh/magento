...
Product.ConfigurableSwatches.prototype = {
    ...
    onOptionClick: function(attr) {
        ...
        if (opt) {
            if (opt != attr._e._last.selectedOption) {
                ...
                var _lb = this.getOptionLabel(opt).split("-");/***/
                _lb[0]=_lb[0].substr(0, 1).toUpperCase() + _lb[0].substr(1);/*make the first letter */
                attr._e.attrLabel.innerHTML = _lb[0];
    ...
    onOptionOver: function() {
        ...
        if(lastOpt && lastOpt.attr.id != opt.attr.id) {
            ...
            lastOpt.attr._e.attrLabel.innerHTML=this.processLabel(lastOpt.attr._e.attrLabel.innerHTML);
        }
    ...
    resetAvailableOptions: function() {
        ...
        if (opt) {
            ...
            attr._e.attrLabel.innerHTML = this.processLabel(attr._e.attrLabel.innerHTML);
    ...
    setStockStatus: function(inStock) {
        if (inStock) {
            this._E.availability.each(function(el) {
                var el = $(el);
                if (typeof _enabled_low_stock === 'undefined'){
