            ...
            try {
                if (k == 'sidebar') {
                    ...
                    if (jQuery('#mini-cart')) {
                        ...
                        jQuery('#sidebar-loading').css('display', 'block');
                        jQuery('#header-cart').css('display', 'block');

                        setTimeout(function () {
                            jQuery('#sidebar-loading').css('display', 'none');
                        }, 900);
                        setTimeout(function () {
    ...
    },
    {
        ...
        afterUpdate: function (args) {
            ...
            if (el.length === 1) {
                ...
                var msg = args[0];
                this._hideBlock();
                this._addMsgBlockProductPage(msg);
        ...
        _addMsgBlockProductPage: function (msg) {
            if (!msg || !msg.length || msg.length < 1) {
                return;
            }
            var msgBlock = $$('.bi-msg-block')[0];
            var msgBlockDynamic = msgBlock.select('li')[0];
            msgBlock.addClassName('messages');
            msgBlock.setStyle({
                'display': 'block'
            });
            if (msgBlockDynamic) {
                msgBlockDynamic.remove();
            }
            msgBlock.appendChild(new Element('li'));
            msgBlock = msgBlock.select('li')[0];
            msgBlock.innerHTML = '';
            ...
            msg.each(function (message) {
                msgBlock.innerHTML += message.text;
            });
        },
