...
(function () {
    ...
    Locator.Detect = Class.create({
        ...
        locateMe: function () {
            ...
            if (self.isValid()) {
                navigator.geolocation.getCurrentPosition(self.pushData, self.handleError, self.settings);
            }
        },
        ...
        pushData: function (position) {
            var data;
            var crd = position.coords;
            if (Locator.defaultSearchSettings.debugMode == true) {
                data = {
                    latitude: -33.710852,
                    longitude: 150.9237386
                };
            } else {
                data = {
            ...
            new Ajax.Request(Locator.defaultSearchSettings.baseUrl + 'locator/search/detect/', {
                method: 'POST',
                parameters: data,
                onSuccess: function (t) {
                    var response = t.responseText.evalJSON(true);
                    jQuery(Locator.defaultSearchSettings.selectors.list).html(response.output);
                }
            });
        },
...
//            });
            $$('.loc-srch button#btn-search').each(function (el) {
                el.addClassName('is-loading');
            });
        },
...
//            });
        ...
        renderLocations: function (locations, coords) {
            ...
            for (var key in locations) {
                if (locations.hasOwnProperty(key)) {
                    ...
                    var markerImage = self.getMarkerImage(l);
                    if (l.isStockist == true) markerImage.url = Locator.defaultSearchSettings.blackImage;
                    if (l.isOnline == true) markerImage.url = Locator.defaultSearchSettings.noneImage;
    ...
    Locator.Search = Class.create({
        ...
        findLocations: function (query, callback) {
            ...
            var url = query.length ? href + '&xhr=1' : href;
            ...
            if (query == "on=store") {
                jQuery('.loc-srch-res-map-wrap').css("visibility", "hidden");
            } else {
                jQuery('.loc-srch-res-map-wrap').css("visibility", "visible");
                jQuery('.loc-srch-res-map-wrap').css("border", "none");
            }
            return this;
        },
        ...
        initEvents: function () {
            ...
            //reset
            $$('#btn-reset').invoke('observe', 'click', function (event) {
                $('locator-autocomplete').value = '';
                $('locator-autocomplete').removeClassName('validation-failed');
                var el = Event.element(event);

                $('btn-reset').addClassName('is-loading');
                self.findLocations({}, function () {
                    $('btn-reset').removeClassName('is-loading');
                });
                Event.stop(event);
            });

            //load more 
            $$(self.settings.selectors.loadmore).invoke('observe', 'click', function (event) {

                var page_size = jQuery('.loc-srch-res-list').find('article.loc-teaser').length + 4;
                var query = window.location.href;
                var params = (query + '&page_size=' + page_size).toQueryParams()
                if (query.indexOf('?') < 0) {
                    params = (window.location.href + '?page_size=' + page_size).toQueryParams();
                }
                jQuery('#loading_img').show();
                jQuery('#loadmore').hide();
                self.findLocations(params, function () {
                    if (typeof (params) === 'object') {
                        for (var key in params) {
                            if (params[key] == 'undefined' || params[key] == '' || key == 'page_size') {
                                delete params[key];
                            }
                        }
                        query = $H(params).toQueryString();
                    }

                    window.history.pushState('', 'Search: ' + jQuery('.loc-srch-res-list').find('article.loc-teaser').length + ' Locations', '?' + query);
                    jQuery('#loading_img').hide();
                    jQuery('#loadmore').show();
                    if (jQuery('.locations').find('article.loc').length >= jQuery('#loc_total').val()) {
                        jQuery('#loadmore').hide();
                    }
                });
                Event.stop(event);
