...
GomageNavigationClass = Class.create({
    ...
    click: function (element) {
        if ($(element)) {
            ...
            this.removeActiveSwatch();

            if (ajax_clean == 1) {
                this.setNavigationUrl(url);
                this.setActiveSwatch(element);
            } else {
    ...
    setActiveSwatch: function (element) {
        var activeClass = 'swatch-active';
        if ($(element)) {
            $(element).addClassName(activeClass);
        }
    },

    removeActiveSwatch: function () {
        var activeClass = 'swatch-active';
        if (jQuery('.swatch-active')) {
            jQuery('.swatch-active').removeClass(activeClass);
        }
    },
