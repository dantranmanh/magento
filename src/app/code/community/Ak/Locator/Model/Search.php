...
class Ak_Locator_Model_Search
{
     ...
     */
    public function search(array $params = array(), $isTotal = false)
    {
        $params = $this->parseParams($params);
        /**fix : can not search with lowercase area param*/
        if(!empty($params['a'])){
            $params['a']=strtoupper($params['a']);
        }
        ...
        foreach ($this->getHandlers() as $key => $handler) {
            ...
            if ($handler->isValidParams($params)) {
                ...
                if($key == 'area') {
                    $handler->setCollection($this->getCollection())->search($params)->getSelect();
                } else {
                    if(!$isTotal)
                        $handler->setCollection($this->getCollection())->search($params)->getSelect();
                    else
    ...
    {
        if (empty($params) || (isset($params['page_size']) && isset($params['xhr']) && count($params)==2)) {
    ...
    public function getCollection()
    {
        if (!$this->_collection) {
            ...
            $page = Mage::app()->getRequest()->getParams();
            if((isset($page['s']))){
                
            }else{
                $this->getCollection()->addAttributeToSort('is_stockist', 'asc');
            }
        }
