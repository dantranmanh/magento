<?php
class Korifi_Base_Helper_Module extends Mage_Core_Helper_Abstract
{
    const INSTALLED_PATH = 'kbase/feed/installed';
    const EXTENSIONS_PATH = 'kbase_extensions';
    const UPDATED_PREFIX = 'kbase/feed/updated_';
    
    const URL_EXTENSIONS  = 'http://www.jetextension.com/feeds/extensions.xml';
    ...
    protected function _isJETModule()
    {
        return strpos($this->_controllerModule, "Korifi") !== FALSE && $this->_getInstalledVersion() !== NULL;
    }
    
    static function baseModuleInstalled()
    {
        $ret = Mage::getStoreConfig(Korifi_Base_Helper_Module::INSTALLED_PATH);
        if (!$ret)
        {
            $ret = time();
            Mage::getConfig()->saveConfig(Korifi_Base_Helper_Module::INSTALLED_PATH, $ret);
    ...
    {
        $path = Korifi_Base_Helper_Module::UPDATED_PREFIX . $this->_controllerModule;
    ...
    {
        $ret = false;
        if ($this->_isJETModule() && $this->_validateBaseModulePeriod() && $this->_validateModulePeriod())
        {
    ...
    {
        return Mage::getStoreConfig('kbase/feed/update') == 1;
    }
}